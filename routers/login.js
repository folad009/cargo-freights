  ...
  try {
    ...
    if (data[0].role == "2") {
      let customer_data = await mySqlQury(
        `SELECT * FROM tbl_customers WHERE email = '${data[0].email}'`,
      );
    ...
    if (data[0].role == "3") {
      let drivers_data = await mySqlQury(
        `SELECT * FROM tbl_drivers WHERE email = '${data[0].email}'`,
      );
...
router.post("/driver_singup", async (req, res) => {
  try {
    ...
    let query =
      "INSERT INTO tbl_admin (first_name, last_name, email, phone_no, password, role) VALUE ('" +
      first_name +
      "', '" +
      last_name +
      "', '" +
