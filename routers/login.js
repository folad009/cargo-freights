...
router.get("/", async (req, res) => {
  try {
    ...
    const customer_data = await mySqlQury(
      `SELECT * FROM tbl_customers WHERE customer_active = '1' ORDER BY id LIMIT 1`,
    );
  ...
  try {
    ...
    if (data[0].role == "2") {
      let customer_data = await mySqlQury(
        "SELECT * FROM tbl_customers WHERE email = ? LIMIT 1",
        [data[0].email],
      );
    ...
    if (data[0].role == "3") {
      let drivers_data = await mySqlQury(
        "SELECT * FROM tbl_drivers WHERE email = ? LIMIT 1",
        [data[0].email],
      );
...
router.get("/country/ajax/:id", async (req, res) => {
  try {
    const state_data = await mySqlQury(
      "SELECT * FROM tbl_states WHERE countries_id = ?",
      [req.params.id],
    );
...
router.get("/state/ajax/:id", async (req, res) => {
  try {
    const city_data = await mySqlQury(
      "SELECT * FROM tbl_city WHERE state_id = ?",
      [req.params.id],
    );
...
router.post("/sign_up", async (req, res) => {
  try {
    ...
    await mySqlQury(
      ...
      [first_name, last_name, email, country_code, phone_no, hash],
    );
    ...
    await mySqlQury(
      `INSERT INTO tbl_customers (first_name, last_name, email, country_code, mobile, customers_country, customers_state, customers_city, customers_zipcode, customers_address, customer_active, login_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?)`,
      [
        first_name,
        last_name,
        email,
...
router.post("/driver_singup", async (req, res) => {
  try {
    ...
    await mySqlQury(
      `INSERT INTO tbl_drivers (first_name, last_name, email, mobile, vehicle_plate, active, login_id) VALUES (?, ?, ?, ?, ?, '0', ?)`,
      [
        first_name,
        last_name,
        email,
...
router.post("/tracking/ajax", async (req, res) => {
  try {
    ...
    const inv = invoice_no != null ? String(invoice_no).trim() : "";
    if (!inv) {
    ...
    const trackingSql = `SELECT tbl_tracking_history.*, (select tbl_countries.countries_name from tbl_countries where tbl_tracking_history.location = tbl_countries.id) as countries_name,
                                                                                ...
                                                                                FROM tbl_tracking_history WHERE invoice = ?`;
    ...
    if (shipment_type == "1") {
      let data = await mySqlQury(
        `SELECT tbl_register_packages.*, (select tbl_customers.first_name from tbl_customers where tbl_register_packages.customer = tbl_customers.id) as customer_firstname,
                                                                        ...
                                                                        FROM tbl_register_packages WHERE invoice = ?`,
        [inv],
      );

      if (!hasRows(data)) return json404();
      ...
      const tracking_data = await mySqlQury(trackingSql, [inv]);
      ...
      if (shipment_type == "1") {
        ...
        const edit_client_data = await mySqlQury(
          "SELECT * FROM tbl_client WHERE id = ?",
          [data[0].client],
        );
      ...
      let data = await mySqlQury(
        `SELECT tbl_shipment.*, (select tbl_customers.first_name from tbl_customers where tbl_shipment.customer = tbl_customers.id) as customer_firstname,
                                                                ...
                                                                FROM tbl_shipment WHERE invoice = ?`,
        [inv],
      );
      ...
      let data = await mySqlQury(
        `SELECT tbl_pickup.*, (select tbl_customers.first_name from tbl_customers where tbl_pickup.customer = tbl_customers.id) as customer_firstname,
      ...
      let data = await mySqlQury(
        `SELECT tbl_consolidated.*, (select tbl_customers.first_name from tbl_customers where tbl_consolidated.customer = tbl_customers.id) as customer_firstname,
                                                                    ...
                                                                    FROM tbl_consolidated WHERE invoice = ?`,
        [inv],
      );
