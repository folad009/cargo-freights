...
router.get("/", auth, async (req, res) => {
  try {
    ...
    if (role_data.role == "1") {
      let registered_packages = await mySqlQury(
        `SELECT SUM(total) AS registered_packages_total FROM tbl_register_packages`,
      );
      let shipments = await mySqlQury(
        `SELECT SUM(total) AS shipments_total FROM tbl_shipment`,
      );
      let pickups = await mySqlQury(
        `SELECT SUM(total) AS pickups_total FROM tbl_pickup`,
      );
      let consolidated = await mySqlQury(
        `SELECT SUM(total) AS consolidated_total FROM tbl_consolidated`,
      );
      ...
      let registered_packages_data =
        await mySqlQury(`SELECT tbl_register_packages.*, (select tbl_customers.first_name from tbl_customers where tbl_register_packages.customer = tbl_customers.id) as customers_firstname,
      ...
      let shipments_data =
        await mySqlQury(`SELECT tbl_shipment.*, (select tbl_customers.first_name from tbl_customers where tbl_shipment.customer = tbl_customers.id) as customers_firstname,
      ...
      let pickups_data =
        await mySqlQury(`SELECT tbl_pickup.*, (select tbl_customers.first_name from tbl_customers where tbl_pickup.customer = tbl_customers.id) as customers_firstname,
      ...
      let consolidated_data =
        await mySqlQury(`SELECT tbl_consolidated.*, (select tbl_customers.first_name from tbl_customers where tbl_consolidated.customer = tbl_customers.id) as customers_firstname,
      ...
      let courier_companies_data = await mySqlQury(
        `SELECT * FROM tbl_courier_companies`,
      );
      let shipping_status_data = await mySqlQury(
        `SELECT * FROM tbl_shipping_status`,
      );
      let logistics_service_data = await mySqlQury(
        `SELECT * FROM tbl_logistics_service`,
      );
      ...
      let registered_packages = await mySqlQury(
        `SELECT SUM(total) AS registered_packages_total FROM tbl_register_packages WHERE customer = '${customer_data[0].id}'`,
      );
      let shipments = await mySqlQury(
        `SELECT SUM(total) AS shipments_total FROM tbl_shipment WHERE customer = '${customer_data[0].id}'`,
      );
      let pickups = await mySqlQury(
        `SELECT SUM(total) AS pickups_total FROM tbl_pickup WHERE customer = '${customer_data[0].id}'`,
      );
      let consolidated = await mySqlQury(
        `SELECT SUM(total) AS consolidated_total FROM tbl_consolidated WHERE customer = '${customer_data[0].id}'`,
      );
      ...
      let registered_packages = await mySqlQury(
        `SELECT SUM(total) AS registered_packages_total FROM tbl_register_packages WHERE assign_driver = '${drivers_data[0].id}'`,
      );
      let shipments = await mySqlQury(
        `SELECT SUM(total) AS shipments_total FROM tbl_shipment WHERE assign_driver = '${drivers_data[0].id}'`,
      );
      let pickups = await mySqlQury(
        `SELECT SUM(total) AS pickups_total FROM tbl_pickup WHERE assign_driver = '${drivers_data[0].id}'`,
      );
      let consolidated = await mySqlQury(
        `SELECT SUM(total) AS consolidated_total FROM tbl_consolidated WHERE assign_driver = '${drivers_data[0].id}'`,
      );
...
router.post("/profile/:id", auth, async (req, res) => {
  try {
    ...
    if (password != "") {
      ...
      await mySqlQury(
        `UPDATE tbl_admin SET first_name = '${first_name}', last_name = '${last_name}', email = '${email}', phone_no = '${phone_no}', password = '${hash}' WHERE id = '${req.params.id}'`,
      );

      await mySqlQury(
        `UPDATE tbl_customers SET first_name = '${first_name}', last_name = '${last_name}', email = '${email}', mobile = '${phone_no}' WHERE login_id = '${req.params.id}'`,
      );
    } else {
      await mySqlQury(
        `UPDATE tbl_admin SET first_name = '${first_name}', last_name = '${last_name}', email = '${email}', phone_no = '${phone_no}' WHERE id = '${req.params.id}'`,
      );
