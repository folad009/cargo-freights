...
exports.APP_VULNS_OPTION = exports.EXCLUDE_APP_VULNS_OPTION = exports.INCLUDE_SYSTEM_JARS_OPTION = exports.CONTAINER_NEW_FACTS_FEATURE_FLAG = exports.DISABLE_CONTAINER_MONITOR_PROJECT_NAME_FIX_FEATURE_FLAG = exports.CONTAINER_CLI_APP_VULNS_ENABLED_FEATURE_FLAG = exports.SCAN_USR_LIB_JARS_FEATURE_FLAG = void 0;
// Feature flag names
...
exports.DISABLE_CONTAINER_MONITOR_PROJECT_NAME_FIX_FEATURE_FLAG = 'disableContainerMonitorProjectNameFix';
exports.CONTAINER_NEW_FACTS_FEATURE_FLAG = 'allowNewContainerFacts';
// CLI option names
...
const uv_1 = __webpack_require__(88004);
const SEPARATOR = '\n-------------------------------------------------------\n';
...
async function monitor(...args0) {
    ...
    if (options.docker) {
        ...
        const disableContainerMonitorProjectNameFix = await (0, feature_flags_1.hasFeatureFlagOrDefault)(constants_1.DISABLE_CONTAINER_MONITOR_PROJECT_NAME_FIX_FEATURE_FLAG, options, false);
    ...
    const hasUvSupport = process.env[uv_1.UV_MONITOR_ENABLED_ENV_VAR] === 'true' &&
        (await (0, feature_flags_1.hasFeatureFlagOrDefault)(package_managers_1.UV_FEATURE_FLAG, options));
    try {
    ...
    if (hasPnpmSupport) {
        featureFlags.add(package_managers_1.PNPM_FEATURE_FLAG);
    }
    if (hasUvSupport) {
        featureFlags.add(package_managers_1.UV_FEATURE_FLAG);
    }
    if (hasImprovedDotnetWithoutPublish) {
        options.useImprovedDotnetWithoutPublish = true;
    }
    const showMavenScope = await (0, feature_flags_1.isFeatureFlagSupportedForOrg)(feature_flags_1.SHOW_MAVEN_BUILD_SCOPE, (0, organization_1.getOrganizationID)());
    if (showMavenScope.ok) {
        featureFlags.add(feature_flags_1.SHOW_MAVEN_BUILD_SCOPE);
    }
    const showScope = await (0, feature_flags_1.isFeatureFlagSupportedForOrg)(feature_flags_1.SHOW_NPM_SCOPE, (0, organization_1.getOrganizationID)());
    if (showScope.ok) {
        featureFlags.add(feature_flags_1.SHOW_NPM_SCOPE);
    }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.filterDockerFacts = exports.isUnmanagedEcosystem = void 0;
const feature_flags_1 = __webpack_require__(63011);
const constants_1 = __webpack_require__(81024);
...
function shouldFilterFact(fact, isOsProject = true) {
...
async function filterDockerFacts(pluginResponse, ecosystem, options) {
    // Only apply filtering for scan results from snyk-docker-plugin
    if (ecosystem !== 'docker') {
        return pluginResponse;
    }
    // Check feature flag - when false, we filter facts
    const includeAllFacts = await (0, feature_flags_1.hasFeatureFlagOrDefault)(constants_1.CONTAINER_NEW_FACTS_FEATURE_FLAG, options, false);
    if (includeAllFacts) {
        return pluginResponse;
    }
    ...
    return {
        ...pluginResponse,
        scanResults: pluginResponse.scanResults.map((scanResult, index) => ({
            ...scanResult,
            facts: scanResult.facts.filter((fact) => !shouldFilterFact(fact, index === 0)),
        })),
    };
}
exports.filterDockerFacts = filterDockerFacts;
...
async function monitorEcosystem(ecosystem, paths, options, contributors) {
    ...
    for (const path of paths) {
        try {
            ...
            const filteredResponse = await (0, common_1.filterDockerFacts)(pluginResponse, ecosystem, options);
        ...
        for (const scanResult of scanResults) {
            try {
                ...
                for (const issuesDataKey in issuesData) {
                    ...
                    if (!(issuesDataKey in issuesDataFiltered)) {
                        ...
                        if (issue) {
                            ...
                            filteredIgnore.push(buildVulnerabilityFromIssue(issueData, issue, packageManager));
                        }
                    }
                }
                (0, plugin_analytics_1.extractAndApplyPluginAnalytics)([
                    {
                        name: 'packageManager',
                        data: (_c = (_b = depGraphData === null || depGraphData === void 0 ? void 0 : depGraphData.pkgManager) === null || _b === void 0 ? void 0 : _b.name) !== null && _c !== void 0 ? _c : '',
                    },
                    {
                    ...
                    },
                    {
                        ...
                        data: (_d = issues.length) !== null && _d !== void 0 ? _d : 0,
                    },
                ]);
                results.push({
...
async function testEcosystem(ecosystem, paths, options) {
    ...
    for (const path of paths) {
        ...
        options.path = path;
        const pluginResponse = await plugin.scan(options);
        const filteredResponse = await (0, common_2.filterDockerFacts)(pluginResponse, ecosystem, options);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.hasFeatureFlagOrDefault = exports.hasFeatureFlag = exports.isFeatureFlagSupportedForOrg = exports.SHOW_NPM_SCOPE = exports.SHOW_MAVEN_BUILD_SCOPE = void 0;
...
exports.SHOW_MAVEN_BUILD_SCOPE = 'show-maven-build-scope';
exports.SHOW_NPM_SCOPE = 'show-npm-scope';
...
var format_vulnerable_result_summary_1 = __webpack_require__(8641);
    ...
    return await monitorRequest({
        body: {
            meta: {
                ...
                org: config_1.default.orgId ||
                    (config_1.default.org ? decodeURIComponent(config_1.default.org) : undefined),
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.buildPluginOptions = void 0;
const feature_flags_1 = __webpack_require__(63011);
/**
 * Returns a shallow clone of the original `options` object with any
 * plugin-specific configuration injected.
 */
async function buildPluginOptions(options) {
    const pluginOptions = { ...options };
    const isGoPackageManager = options.packageManager === 'gomodules' ||
        options.packageManager === 'golangdep';
    if (isGoPackageManager) {
        const includeGoStandardLibraryDeps = await (0, feature_flags_1.hasFeatureFlagOrDefault)('includeGoStandardLibraryDeps', options, false);
        const disableGoPackageUrls = await (0, feature_flags_1.hasFeatureFlagOrDefault)('disableGoPackageUrlsInCli', options, false);
...
exports.buildPluginOptions = buildPluginOptions;
...
async function getDepsFromPlugin(root, options, featureFlags = new Set()) {
    ...
    const inspectRes = await (0, get_single_plugin_result_1.getSinglePluginResult)(root, options, '', featureFlags);
...
const detect_1 = __webpack_require__(45318);
const package_managers_1 = __webpack_require__(53847);
const { SHOW_NPM_SCOPE } = __webpack_require__(63011);
const get_single_plugin_result_1 = __webpack_require__(8598);
const convert_single_splugin_res_to_multi_custom_1 = __webpack_require__(99695);
const convert_multi_plugin_res_to_multi_custom_1 = __webpack_require__(23110);
const errors_1 = __webpack_require__(55191);
...
async function getMultiPluginResult(root, options, targetFiles, featureFlags = new Set()) {
    ...
    if (featureFlags.has(package_managers_1.PNPM_FEATURE_FLAG)) {
        const { scannedProjects: scannedPnpmResults, unprocessedFiles } = await processWorkspacesProjects(root, options, targetFiles, 'pnpm', featureFlags);
        unprocessedFilesfromWorkspaces = unprocessedFiles;
        allResults.push(...scannedPnpmResults);
    }
    const { scannedProjects: scannedYarnResults, unprocessedFiles: unprocessedFilesFromYarn, } = await processWorkspacesProjects(root, options, unprocessedFilesfromWorkspaces, 'yarn', featureFlags);
    allResults.push(...scannedYarnResults);
    const { scannedProjects: scannedNpmResults, unprocessedFiles } = await processWorkspacesProjects(root, options, unprocessedFilesFromYarn, 'npm', featureFlags);
    ...
    for (const targetFile of unprocessedFiles) {
        ...
        try {
            const inspectRes = await (0, get_single_plugin_result_1.getSinglePluginResult)(root, optionsClone, optionsClone.file, featureFlags);
            ...
            if (error.name === 'NotSupportedEcosystem' ||
                ((_a = error.constructor) === null || _a === void 0 ? void 0 : _a.name) === 'NotSupportedEcosystem') {
                debug(chalk_1.default.bold.yellow(`\n${theme_1.icon.INFO} Skipping unsupported ecosystem for ${targetFile}: ${error.message}`));
                continue;
            }
            const errMessage = (_b = error.message) !== null && _b !== void 0 ? _b : 'Something went wrong getting dependencies';
...
async function processWorkspacesProjects(root, options, targetFiles, packageManager, featureFlags = new Set()) {
    try {
...
const build_plugin_options_1 = __webpack_require__(13528);
const { SHOW_MAVEN_BUILD_SCOPE, SHOW_NPM_SCOPE } = __webpack_require__(63011);
async function getSinglePluginResult(root, options, targetFile, featureFlags = new Set()) {
    ...
    const pluginOptions = await (0, build_plugin_options_1.buildPluginOptions)(options);
    const inspectRes = await moduleInfo.inspect(root, targetFile || options.file, {
        ...pluginOptions,
        showMavenBuildScope: featureFlags.has(SHOW_MAVEN_BUILD_SCOPE),
        showNpmScope: featureFlags.has(SHOW_NPM_SCOPE),
    }, snyk_http_client_1.snykHttpClient);
    return inspectRes;
}
...
const uvPlugin = __webpack_require__(88004);
const errors_1 = __webpack_require__(55191);
...
async function parse(root, targetFile, options) {
    ...
    try {
        ...
        return lockFileParser.buildDepTreeFromFiles(root, manifestFileFullPath, lockFileFullPath, options.dev, strictOutOfSync, true, options.showNpmScope);
    }
    finally {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.inspect = exports.UV_MONITOR_ENABLED_ENV_VAR = void 0;
const dep_graph_1 = __webpack_require__(71479);
exports.UV_MONITOR_ENABLED_ENV_VAR = 'SNYK_INTERNAL_UV_MONITOR_ENABLED';
const STUB_DEP_GRAPH_DATA = {
    schemaVersion: '1.3.0',
    pkgManager: {
        name: 'pip',
    },
...
async function inspect(_root, targetFile, _options = {}) {
    if (process.env[exports.UV_MONITOR_ENABLED_ENV_VAR] !== 'true') {
        throw new Error(`uv monitor support is not yet available.`);
    }
    const depGraph = (0, dep_graph_1.createFromJSON)(STUB_DEP_GRAPH_DATA);
    return {
        plugin: {
            name: 'snyk-uv-plugin',
            runtime: process.version,
...
async function assembleEcosystemPayloads(ecosystem, options) {
    ...
    try {
        ...
        pluginResponse = await (0, common_1.filterDockerFacts)(pluginResponse, ecosystem, options);
        const payloads = [];
        ...
        for (const scanResult of pluginResponse.scanResults) {
            ...
            payloads.push({
                ...
                qs: (0, common_2.assembleQueryString)(options),
            });
        }
        return payloads;
    }
...
async function runTest(projectType, root, options, featureFlags = new Set()) {
    ...
    try {
        ...
        if (!options.docker &&
            ((0, common_1.shouldPrintDepGraph)(options) || (0, common_1.shouldPrintEffectiveDepGraph)(options))) {
            const results = [];
            return results;
        }
        ...
        throw new errors_1.FailedToRunTestError(error.userMessage ||
            error.message ||
            `Failed to test ${projectType} project`, error.code, error.innerError, error instanceof error_catalog_nodejs_public_1.ProblemError ? error : error.errorCatalog);
    }
    finally {
        spinner_1.spinner.clear(spinnerLbl)();
    }
...
async function assembleLocalPayloads(root, options, featureFlags = new Set()) {
    ...
    try {
        ...
        if (failedResults === null || failedResults === void 0 ? void 0 : failedResults.length) {
            ...
            if ((0, common_1.shouldPrintEffectiveDepGraphWithErrors)(options)) {
                for (const failed of failedResults) {
                    await (0, common_1.printEffectiveDepGraphError)(root, failed, process.stdout);
                }
            }
        ...
        for (const scannedProject of deps.scannedProjects) {
            ...
            if ((0, common_1.shouldPrintEffectiveDepGraph)(options)) {
                spinner_1.spinner.clear(spinnerLbl)();
                await (0, common_1.printEffectiveDepGraph)(depGraph.toJSON(), targetFile, project.plugin.targetFile, target, process.stdout);
            }
